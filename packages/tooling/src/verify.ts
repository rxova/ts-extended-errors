...
export const STEPS: [name: string, command: string][] = [
  ...
  // Reads the patch against origin/main for skipped tests, weakened assertions
  // and lowered thresholds. Before the tests, because a green run that only
  // went green by deleting a test is the case it exists for.
  ['test integrity', 'pnpm run check:integrity'],
  // The other half: implementations that only pass because they hardcode the
  // answer a test expects, or return a stub. basting is vendored (not on npm);
  // see vendor/basting/PROVENANCE.json for where the build came from.
  ['fake implementations', 'pnpm run check:faking'],
